...
class EnhancedBollingerBandsStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        for asset in self.assets:
            ...
            self.bollinger_bands[symbol] = self.BB(symbol, 20, 2, Resolution.Daily, Field.Close)
            self.SetWarmUp(20)
    ...
    def OnData(self, data):
        ...
        for symbol in self.assets:
            ...
            if price < bb.LowerBand.Current.Value and not self.Portfolio[symbol].Invested:
                self.SetHoldings(symbol, self.CalculatePositionSize(portfolio_value, symbol))
            elif price > bb.UpperBand.Current.Value and self.Portfolio[symbol].Invested:
